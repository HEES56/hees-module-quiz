...
import { assignUserGrade, formatUserData } from "./serialization";
import { serializeUser,} from "./transformation";
...
const processUserData = (user) => {
  ...
  const gradUser = assignUserGrade(user);
  const formatUser = formatUserData(gradUser);
  const serialUser = serializeUser(formatUser);
  return serialUser;
  } catch(err) {
    console.log('err', err);
  }
